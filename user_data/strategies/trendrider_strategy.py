...
Fork adaptations (spot / Binance.US paper path — not a rewrite of entry logic):
- BTC market-sentiment informative pair is spot `BTC/USDT` (upstream used
  Bybit perpetual `BTC/USDT:USDT`).
- Intended config is this fork's Binance.US spot dry-run / research JSON
  (empty keys, dry_run true). Upstream config.example.json targeted Bybit
  futures isolated margin with stoploss_on_exchange and placeholder API keys.
- Telegram/Cornix signal formatting is unchanged; DataProvider.send_msg is a
  no-op in backtesting. Do not enable live Telegram with secrets in git.
"""
...
class TrendRiderStrategy(IStrategy):
    ...
    # --- Leverage: 1x for Strat Ninja (spot-safe; unused on spot) ---
    leverage_value = 1

    # Upstream Bybit futures pair was BTC/USDT:USDT. Spot research uses BTC/USDT.
    btc_sentiment_pair = "BTC/USDT"
    ...
    def informative_pairs(self):
        ...
        # BTC as market sentiment (spot pair; upstream used BTC/USDT:USDT)
        informative.append((self.btc_sentiment_pair, "1h"))
        informative.append((self.btc_sentiment_pair, "4h"))
        return informative
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.dp:
            ...
            # BTC market sentiment (spot BTC/USDT; upstream used BTC/USDT:USDT)
            df_btc = self.dp.get_pair_dataframe(pair=self.btc_sentiment_pair, timeframe='1h')
            if len(df_btc) > 0:
